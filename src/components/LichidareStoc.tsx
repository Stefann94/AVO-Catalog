 ...
 * ─── DE UNDE VINE ─────────────────────────────────────────────────────────
 *
 * Fișierul ăsta ERA OferteleLunii.tsx. Banda arăta „cele mai bune oferte",
 * calculate de noi din diferența dintre prețul de listă și cel de volum — și
 * așa ajungeau în ea 15 invertoare pe care catalogul nu le pune la ofertă
 * nicăieri. O selecție pe care n-o face furnizorul, sub un titlu care sugerează
 * că o face, nu mai avea loc.
 *
 * Catalogul marchează el însuși două lucruri: pagina „OFERTELE LUNII" (patru
 * produse, acum în OferteleLunii.tsx, imediat dedesubt) și secțiunile
 * „LICHIDARE STOC" (15 produse). Pentru bandă s-a ales a doua, fiindcă e
 * singura listă din catalog destul de lungă cât săgețile să aibă ce derula.
 *
 * S-au schimbat DOAR titlul, subtitlul și sursa produselor. Designul —
 * masthead, ștampila, banda, cardul, subsolul, spațierea — e neatins, iar
 * explicațiile de mai jos rămân valabile ca atare.
 *
 * ─── DESIGNUL, MOȘTENIT DE LA „OFERTELE LUNII" ────────────────────────────
 *
 * ══════════════════════════════════════════════════════════════════════════
 * DE CE ARATĂ AȘA
 * ──────────────────────────────────────────────────────────────────────────
...
export default async function LichidareStoc({
  oferte,
}: {
  ...
  const [perioada, lista] = await Promise.all([
    ...
    oferte ? Promise.resolve(oferte) : incarcaLichidareStoc(),
  ]);

  // Fără produse nu există secțiune. O bandă goală sub un titlu ar anunța ceva
  // ce nu livrează; în ediția în care catalogul nu mai are nimic la lichidare,
  // secțiunea pur și simplu nu apare.
  if (lista.length === 0) return null;

  // Fără luna în titlu, spre deosebire de „Ofertele lunii": lichidarea nu se
  // reînnoiește lunar, ține cât ține stocul. Textul e identic cu al paginii
  // /catalog/lichidare-stoc, ca banda și destinația ei să se numească la fel.
    ...
    <section className="bg-white py-16 lg:pt-12 lg:pb-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-12">
        ...
        {/* ── Banda de oferte ────────────────────────────────────
            ...
            exact patru produse — cele de pe coperta catalogului. Aici ajung
            produsele din secțiunile „LICHIDARE STOC" (vezi lib/oferte.ts),
            deci numărul variază de la o ediție la alta: pe catalogul curent
            sunt 15. O grilă
